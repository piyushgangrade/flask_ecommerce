...
def buy_now(product_id):
    try:
        if models.Cart.get((models.Cart.user_email == current_user.id) & (models.Cart.product_id == product_id)):
            ...
            q = models.Cart.update(count = prod.count + 5).where((models.Cart.user_email == current_user.id) & (models.Cart.product_id == product_id))
            q.execute()
    ...
    if request.method == 'POST':
        ...
        for product in products:
            models.BuyHistory.add_history(
                ...
                buyer_name = fullname,
                buyer_address = 'Delivery Address: ' + hostelname,
                mobile_no = mobile_no,
